...
@property (nonatomic, assign) NSTimeInterval netStartTime;  //请求发起时间

#pragma mark **************httpbody**************
@property (nonatomic, strong) NSData *httpbody;             //httpbody
...
- (NSString *)patternUrl; //模板化的url，用于如/v3/devices/%@/ptzControll等url的匹配。
- (BOOL)useCDN; // 是否使用CDN的host地址
- (NSTimeInterval)timeoutInterval; // 超时时间，默认为20秒
...
- (NSSet<NSString *> *)customMethodsEncodingParametersInURI; // 自定义哪些 http 方法进行参数编码
...
//根据返回的错误码弹窗
- (void)alertErrorMessageWith:(NSString *)errorMsg;
